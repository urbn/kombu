...
class Channel(virtual.Channel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        logger.debug('There are %s queues' % len(queues))
        for queue in queues:
            logger.debug('Next queue: %s' % queue)
    ...
    def _message_to_python(self, message, queue_name, queue):
        ...
        logger.info('The body is %s' % body)
        payload = loads(body)
    ...
    #TODO: Remove all of the region discovery code
    def _aws_connect_to(self, fun, regions):
        from boto.sqs import SQSRegionInfo
        logger.debug('conninfo is: {userid: %s, password: %s, hostname: %s, port: %s' %
                     (self.conninfo.userid, self.conninfo.password , self.conninfo.hostname, self.conninfo.port))
